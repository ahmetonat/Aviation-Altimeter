...
//I2C1 calls were converted to I2C2 calls for Naze32. 
// Old I2C1 calls retained for completeness.
...
void ms5611_init (struct ms5611_vars * baro){
  ...
  I2C_Write (I2C2, command+3, 1, MS5611ADDR); //Reset MS5611
  //Reset sequence takes 2.8ms.
  Delay(3);
  
  for (i=0;i<8;++i){
    //I2C_Write (I2C1, command+4+i, 1, MS5611ADDR); //Set register to be read
    I2C_Write (I2C2, command+4+i, 1, MS5611ADDR); //Set register to be read
    //I2C_Read (I2C1, I2Cregister, 2, MS5611ADDR);
    I2C_Read (I2C2, I2Cregister, 2, MS5611ADDR);
...
void ms5611_measure(struct ms5611_vars * baro){
  ...
  I2C_Write (I2C2, command+1, 1, MS5611ADDR); //Start ADC for pressure.
  Delay(10);                                  //Blocking wait for ADC end.
  //I2C_Write (I2C1, command, 1, MS5611ADDR);   //Command: Read ADC
  I2C_Write (I2C2, command, 1, MS5611ADDR);   //Command: Read ADC
  //I2C_Read (I2C1, I2Cregister, 3, MS5611ADDR);
  I2C_Read (I2C2, I2Cregister, 3, MS5611ADDR);
  ...
  I2C_Write (I2C2, command+2, 1, MS5611ADDR); //Start ADC for temperature.
  Delay(10);                                  //Blocking wait for ADC end.
  //I2C_Write (I2C1, command, 1, MS5611ADDR);   //Command: Read ADC
  I2C_Write (I2C2, command, 1, MS5611ADDR);   //Command: Read ADC
  //I2C_Read (I2C1, I2Cregister, 3, MS5611ADDR);
...
void ms5611_calculate(struct ms5611_vars * baro){
  ...
  if (temp < 2000) { // temperature lower than 20degC
    ...
    if (temp < -1500) { // temperature lower than -15degC
      delt = temp + 1500;       //TODO: Check this.
      delt = delt * delt;
